...
<#
.SYNOPSIS
    Build Intent Forge template release archives for each supported AI assistant and script type.
...
function Rewrite-Paths {
    ...
    $Content = $Content -replace '(/?)\bmemory/', '.intent/memory/'
    $Content = $Content -replace '(/?)\bscripts/', '.intent/scripts/'
    $Content = $Content -replace '(/?)\btemplates/', '.intent/templates/'
    return $Content
}
...
function Generate-Commands {
    ...
    foreach ($template in $templates) {
        ...
        $outputFile = Join-Path $OutputDir "intent.$name.$Extension"
...
function Generate-CopilotPrompts {
    ...
    $agentFiles = Get-ChildItem -Path "$AgentsDir/intent.*.agent.md" -File -ErrorAction SilentlyContinue
...
function Build-Variant {
    ...
    $specDir = Join-Path $baseDir ".intent"
    New-Item -ItemType Directory -Path $specDir -Force | Out-Null
    
    # Copy memory directory
    if (Test-Path "memory") {
        Copy-Item -Path "memory" -Destination $specDir -Recurse -Force
        Write-Host "Copied memory -> .intent"
    }
    ...
    if (Test-Path "templates") {
        ...
        Write-Host "Copied templates -> .intent/templates"
    }
    ...
    # Create zip archive
    $zipFile = Join-Path $GenReleasesDir "intent-template-${Agent}-${Script}-${Version}.zip"
...
Get-ChildItem -Path $GenReleasesDir -Filter "intent-template-*-${Version}.zip" | ForEach-Object {
    Write-Host "  $($_.Name)"
}