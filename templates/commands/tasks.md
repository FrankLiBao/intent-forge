---
description: 根据可用的设计工件为功能生成可操作的、依赖排序的 tasks.md
handoffs:
...
1. **设置**：从仓库根目录运行 `scripts/bash/check-prerequisites.sh --json` 并解析 FEATURE_DIR 和 AVAILABLE_DOCS 列表。所有路径必须是绝对路径。对于参数中的单引号（如 "I'm Groot"），使用转义语法：例如 'I'\''m Groot'（或者如果可能，使用双引号："I'm Groot"）。
...
4. **生成 tasks.md**：使用 `.intent/templates/tasks-template.md` 作为结构，填写：
   - 来自 plan.md 的正确功能名称
   - 阶段 1：设置任务（项目初始化）
