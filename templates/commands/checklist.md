...
- ✅ "'突出显示'是否用具体的大小/位置进行了量化？"（清晰度）
- ✅ "悬停状态要求在所有交互元素中是否一致？"（一致性）
- ✅ "是否为键盘导航定义了无障碍要求？"（覆盖率）
- ✅ "规范是否定义了徽标图像加载失败时的情况？"（边界情况）

**比喻**：如果您的规范是用英语编写的代码，那么检查清单就是它的单元测试套件。您正在测试需求是否编写良好、完整、明确并准备好实现 - 而不是测试实现是否有效。
...
1. **设置**：从仓库根目录运行 `scripts/bash/check-prerequisites.sh --json` 并解析 JSON 以获取 FEATURE_DIR 和 AVAILABLE_DOCS 列表。
   - 所有文件路径必须是绝对路径。
   - 对于参数中的单引号（如 "I'm Groot"），使用转义语法：例如 'I'\''m Groot'（或者如果可能，使用双引号："I'm Groot"）。

2. **澄清意图（动态）**：最多派生三个初始上下文澄清问题（无预先准备的目录）。它们必须：
   ...
   生成算法：
   1. 提取信号：功能领域关键词（例如，auth、latency、UX、API）、风险指标（"关键"、"必须"、"合规"）、利益相关者提示（"QA"、"审查"、"安全团队"）和明确的交付成果（"a11y"、"回滚"、"契约"）。
   2. 将信号聚类为候选焦点区域（最多4个），按相关性排序。
   3. 如果不明确，识别可能的受众和时间（作者、审查者、QA、发布）。
   4. 检测缺失的维度：范围广度、深度/严谨性、风险强调、排除边界、可测量的验收标准。
   5. 从这些原型中选择制定问题：
   ...
   输出问题（标记Q1/Q2/Q3）。回答后：如果≥2个场景类（备选/异常/恢复/非功能领域）仍不清楚，您可以提出最多两个更有针对性的后续问题（Q4/Q5），每个都有一行理由（例如，"未解决的恢复路径风险"）。不要超过总共五个问题。如果用户明确拒绝更多，则跳过升级。
...
5. **生成检查清单** - 创建"需求的单元测试"：
   ...
   - 生成唯一的检查清单文件名：
     ...
     - 如果文件存在，附加到现有文件
   - 从 CHK001 开始按顺序编号项目
   - 每次 `/intent.checklist` 运行都会创建一个新文件（永远不会覆盖现有检查清单）
   ...
   **按质量维度的示例**：
   ...
   - "卡片组件要求在着陆页和详细页面之间是否一致？[一致性]"

   覆盖率：
   - "是否为零状态场景（无剧集）定义了要求？[覆盖率, 边界情况]"
   - "是否涵盖并发用户交互场景？[覆盖率, Gap]"
   ...
   **场景分类和覆盖率**（需求质量焦点）：
   ...
   - 如果缺少场景类："[场景类型] 要求是故意排除还是缺失？[Gap]"
   - 当发生状态突变时包括弹性/回滚："是否为迁移失败定义了回滚要求？[Gap]"

   **可追溯性要求**：
   - 最低限度：≥80% 的项目必须至少包含一个可追溯性引用
...
6. **结构参考**：遵循 `.intent/templates/checklist-template.md` 中的规范模板生成检查清单，用于标题、元部分、类别标题和 ID 格式。如果模板不可用，使用：H1 标题、目的/创建元行、包含 `- [ ] CHK### <需求项>` 行的 `##` 类别部分，从 CHK001 开始全局递增 ID。
...
**重要提示**：每次调用 `/intent.checklist` 命令都会使用简短的描述性名称创建检查清单文件，除非文件已存在。这允许：
...
```markdown
- [ ] CHK001 - 验证着陆页显示3张剧集卡片 [Spec §FR-001]
...
- [ ] CHK006 - "视觉层次"要求是否可以客观测量？[可测量性, Spec §FR-001]
```
