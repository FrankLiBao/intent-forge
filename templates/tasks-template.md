...
**测试**：下面的示例包含测试任务。测试是可选的 - 仅在功能规范中明确请求时才包含它们。
...
<!--
  ...
  /intent.tasks 命令必须根据以下内容将这些替换为实际任务：
...
- **用户故事 2 (P2)**：可以在基础（阶段 2）后开始 - 可能与 US1 集成，但应该可以独立测试
- **用户故事 3 (P3)**：可以在基础（阶段 2）后开始 - 可能与 US1/US2 集成，但应该可以独立测试
