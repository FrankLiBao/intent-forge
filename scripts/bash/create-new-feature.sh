...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "  --short-name <name> 提供自定义简短名称（2-4个词）用于分支"
...
# 检查现有分支（本地和远程）并返回下一个可用编号
check_existing_branches() {
    local specs_dir="$1"

    # 获取所有远程仓库以获取最新分支信息（如果没有远程仓库则抑制错误）
    git fetch --all --prune 2>/dev/null || true

    # 从所有分支获取最高编号（不仅仅是匹配简短名称的）
    local highest_branch=$(get_highest_from_branches)

    # 从所有 specs 获取最高编号（不仅仅是匹配简短名称的）
...
# 解析仓库根目录。优先使用 git 信息（如果可用），但回退到
# 搜索仓库标记，以便工作流在使用 --no-git 初始化的仓库中仍能正常工作。
...
generate_branch_name() {
    ...
    # 过滤单词：删除停用词和短于 3 个字符的单词（除非它们是原始的大写首字母缩略词）
    local meaningful_words=()
    for word in $clean_name; do
        # 跳过空单词
        [ -z "$word" ] && continue

        # 保留不是停用词且（长度 >= 3 或是潜在首字母缩略词）的单词
        if ! echo "$word" | grep -qiE "$stop_words"; then
            if [ ${#word} -ge 3 ]; then
                meaningful_words+=("$word")
            elif echo "$description" | grep -q "\b${word^^}\b"; then
                # 如果短单词在原始文本中以大写形式出现（可能是首字母缩略词），则保留
                meaningful_words+=("$word")
            fi
        fi
    done
...
# 强制使用十进制解释以防止八进制转换（例如，010 → 八进制为 8，但应该是十进制为 10）
...
TEMPLATE="$REPO_ROOT/.intent/templates/spec-template.md"
SPEC_FILE="$FEATURE_DIR/spec.md"
if [ -f "$TEMPLATE" ]; then cp "$TEMPLATE" "$SPEC_FILE"; else touch "$SPEC_FILE"; fi

# 为当前会话设置 INTENT_FEATURE 环境变量
...
if $JSON_MODE; then
    ...
    echo "INTENT_FEATURE 环境变量设置为: $BRANCH_NAME"
fi
